...
export function render(outlet) {
  outlet.innerHTML = html`
    ...
    <section class="panel wrap">
      <!-- THE ARGUMENT BEFORE THE PRICES. Somebody who reads the bands
           first compares them to a competitor's bands; somebody who
           reads this first understands why the comparison does not
           apply. -->
      ...
      <!-- NO NEW CLASSES ON THIS PAGE, and the reason is the standing
           rule rather than laziness. CSS is at 59.9 KB of 60 and the
           receipt from three changes ago says the next raise splits the
           stylesheet first. Four rules were written here — a grid, a
           price, and two list treatments — and every one of them
           already existed under another name: the risk picture's cards,
           its grid, the stat treatment its figures use, and the plain
           statement list under "what to do next". Reusing them costs nothing and makes the pricing page
           look like the product rather than like a landing page bolted
           to it. -->
      <div class="picture-grid">
        ${BANDS.map(
          (b) => html`<article class="card">
            ...
            <!-- NOT THE STAT TREATMENT, and the sweep is why. Reusing it here put
                 Savannah Gold on a white card at 2.16:1 and its label at
                 1.09:1 — not low contrast but INVISIBLE. That treatment is
                 written for the dark band, where gold on charcoal is
                 the identity; on this card it is the exact defect the
                 accessibility sweep exists to catch, a class written
                 for a dark surface reused on a light one. The price is
                 ordinary emphasised text instead, which reads at the
                 full ink contrast and costs no new rule. -->
            <p class="cov__has">
              <strong>$${b.usdMonthly}</strong> a month, whole operator
            </p>
      ...
      <!-- WHAT IS NOT DECIDED, on the page rather than in a footnote.
           An operator asking "can I pay by M-Pesa" deserves the honest
           answer on the pricing page, and a product that has spent this
           much effort refusing to overstate its coverage cannot start
